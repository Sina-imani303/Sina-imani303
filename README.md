# Hi 👋 I'm Sina

### Frontend Developer from Iran 🇮🇷

I build modern, responsive and high-performance web applications using React, Next.js and TypeScript.
## 🇮🇷 درباره من
سلام، من سینا ایمانی هستم، توسعه‌دهنده فرانت‌اند با تمرکز بر React، Next.js و TypeScript. به طراحی و توسعه وب‌سایت‌ها و وب‌اپلیکیشن‌های مدرن، سریع و واکنش‌گرا علاقه‌مندم و آماده همکاری در پروژه‌های داخلی و بین‌المللی هستم.

## 💻 Tech Stack

- React.js
- Next.js
- JavaScript (ES6+)
- TypeScript
- HTML5
- CSS3
- Tailwind CSS
- Bootstrap
- Redux Toolkit
- TanStack Query
- REST API
- Axios
- AJAX
- GSAP
- Three.js
- Git
- GitHub
- Webpack
- Responsive Design
- PWA

## 📫 Contact

- 🌐 Portfolio: https://portfolio-sina-imani.vercel.app
- 💼 LinkedIn: https://linkedin.com/in/...
- 💬 Telegram: https://t.me/Sina_immani
- 📧 Email: sinaiamnii20@gmail.com
